...
const ThumbnailFull: React.FC<IProps> = ({ open, num, numPages, setOpen, setNum }) => {
  /**
   * 关闭
   */
  ...
  return createPortal(
    <div
      className={cn(
        'fixed top-0 bottom-0 left-0 right-0 bg-[rgba(0,0,0,0.5)]',
        open ? 'flex-center' : 'hidden'
      )}
    >
      {/* 关闭按钮 */}
      ...
      {/* 浏览 */}
      <Thumbnail pageNumber={numRef.current} width={550} className="relative cursor-default" />
